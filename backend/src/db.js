...
const usePostgres = configured(process.env.DATABASE_URL);
const dbMode = usePostgres ? 'postgres' : 'sqlite';
...
function toPostgresSql(sql) {
  let text = sql
    ...
    .replace(/INSERT OR IGNORE INTO\s+list_collaborators\s+\(list_id,\s*user_id\)\s+VALUES\s+\(\?,\s*\?\)/i,
      'INSERT INTO list_collaborators (list_id, user_id) VALUES (?, ?) ON CONFLICT DO NOTHING')
    .replace(/INSERT OR REPLACE INTO\s+list_collaborators\s+\(list_id,\s*user_id,\s*role\)\s+VALUES\s+\(\?,\s*\?,\s*\?\)/i,
      'INSERT INTO list_collaborators (list_id, user_id, role) VALUES (?, ?, ?) ON CONFLICT (list_id, user_id) DO UPDATE SET role = EXCLUDED.role');

  text = convertPlaceholders(text);

  // Postgres lower-cases unquoted aliases. Keep the existing route contract.
  text = text.replace(/SELECT MAX\(position\) as m FROM/i, 'SELECT MAX(position) AS m FROM');
  return text;
}
...
function createPostgresDb() {
  ...
  return {
    ...
    async exec(sql) {
      return pool.query(toPostgresSql(sql));
    },
    async close() {
      return pool.end();
    },
...
function createSqliteDb() {
  ...
  ensureColumn('receipt_images', 'storage_path', 'TEXT');
  ensureColumn('receipt_images', 'image_data', 'TEXT');
