...
async function canAccess(listId, userId) {
...
router.post('/', async (req, res) => {
  ...
  res.json(await db.prepare('SELECT * FROM shopping_lists WHERE id = ?').get(id));
});

// GET /api/lists/:id
router.get('/:id', async (req, res) => {
  const access = await canAccess(req.params.id, req.user.userId);
...
router.patch('/:id', async (req, res) => {
  ...
  const { name } = req.body;
  if (name?.trim()) {
    await db.prepare('UPDATE shopping_lists SET name = ? WHERE id = ?').run(name.trim(), req.params.id);
  }
  res.json(await db.prepare('SELECT * FROM shopping_lists WHERE id = ?').get(req.params.id));
});
...
router.delete('/:id', async (req, res) => {
  const access = await canAccess(req.params.id, req.user.userId);
  if (!access || access.role !== 'owner') return res.status(403).json({ error: 'Not authorized' });
  await db.prepare('DELETE FROM shopping_lists WHERE id = ?').run(req.params.id);
  res.json({ ok: true });
});
...
router.post('/:id/join', async (req, res) => {
  ...
  await db.prepare('INSERT OR IGNORE INTO list_collaborators (list_id, user_id) VALUES (?, ?)')
...
router.post('/:id/invite', async (req, res) => {
  ...
  const invitee = await db.prepare('SELECT id FROM users WHERE email = ?').get(email?.toLowerCase());
  ...
  await db.prepare('INSERT OR REPLACE INTO list_collaborators (list_id, user_id, role) VALUES (?, ?, ?)')
...
router.delete('/:id/collaborators/:userId', async (req, res) => {
  ...
  await db.prepare('DELETE FROM list_collaborators WHERE list_id = ? AND user_id = ?')
...
router.post('/:id/items', async (req, res) => {
  ...
  const id = nanoid();
  const maxPos = await db.prepare('SELECT MAX(position) as m FROM list_items WHERE list_id = ?').get(req.params.id);
  await db.prepare(`INSERT INTO list_items
...
router.patch('/:id/items/:itemId', async (req, res) => {
  ...
  await db.prepare(`UPDATE list_items SET
    ...
    checked !== undefined ? Boolean(checked) : null,
    quantity ?? null, notes ?? null, store_choice ?? null, position ?? null,
    req.params.itemId, req.params.id,
  );

  res.json(await db.prepare('SELECT * FROM list_items WHERE id = ?').get(req.params.itemId));
});
...
router.delete('/:id/items/:itemId', async (req, res) => {
  const access = await canAccess(req.params.id, req.user.userId);
  if (!access || access.role === 'viewer') return res.status(403).json({ error: 'Not authorized' });
  await db.prepare('DELETE FROM list_items WHERE id = ? AND list_id = ?').run(req.params.itemId, req.params.id);
  res.json({ ok: true });
});
