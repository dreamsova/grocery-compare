...
router.post('/refresh', async (req, res) => {
  ...
  const { accessToken, refreshToken } = await createTokens(stored.user_id);
  setCookies(res, accessToken, refreshToken);
  res.json({ ok: true });
});

router.get('/me', requireAuth, async (req, res) => {
  const user = await db.prepare('SELECT id, email, display_name, ic_plus, wm_plus FROM users WHERE id = ?')
    .get(req.user.userId);
...
router.patch('/me', requireAuth, async (req, res) => {
  ...
  await db.prepare(`UPDATE users SET
    ...
    WHERE id = ?`).run(
    display_name ?? null,
    ic_plus !== undefined ? Boolean(ic_plus) : null,
    wm_plus !== undefined ? Boolean(wm_plus) : null,
    req.user.userId,
  );
  const user = await db.prepare('SELECT id, email, display_name, ic_plus, wm_plus FROM users WHERE id = ?')
    .get(req.user.userId);
