...
import { loadReceiptImage, storeReceiptImage } from '../storage/receipts.js';
...
async function latestPriceFor(productId, store) {
...
async function latestPrices(productId) {
  return {
    kroger: await latestPriceFor(productId, 'kroger'),
    community: await db.prepare(
      `SELECT ${PRICE_FIELDS} FROM price_snapshots
       WHERE product_id = ?
         AND store IN ('manual', 'aldi', 'costco', 'trader_joes')
       ORDER BY scraped_at DESC LIMIT 1`
    ).get(productId) ?? null,
    walmart: await latestPriceFor(productId, 'walmart'),
    instacart: await latestPriceFor(productId, 'instacart'),
  };
}
...
async function receiptRows(productId) {
  const rows = await db.prepare(
    `SELECT id, product_id, price_snapshot_id, store, storage_path, mime_type, file_name, file_size, note, uploaded_by, created_at
     FROM receipt_images
     WHERE product_id = ?
...
router.get('/', async (req, res) => {
  ...
  const products = q
    ? await db.prepare(`SELECT * FROM products WHERE name LIKE ? LIMIT ? OFFSET ?`)
        .all(`%${q}%`, +limit, +offset)
    : await db.prepare(`SELECT * FROM products WHERE created_by = ? LIMIT ? OFFSET ?`)
        .all(req.user.userId, +limit, +offset);

  const serialized = await Promise.all(products.map(async p => ({
    ...serializeProduct(p),
    prices: await latestPrices(p.id),
  })));
  res.json(serialized);
});
...
router.post('/', async (req, res) => {
  ...
  const existing = await db.prepare('SELECT * FROM products WHERE name = ? AND created_by = ?')
  ...
  if (existing) {
    ...
    const product = await db.prepare('SELECT * FROM products WHERE id = ?').get(existing.id);
    return res.json({ ...serializeProduct(product), prices: await latestPrices(existing.id) });
  }
  ...
  res.json({ ...serializeProduct(product), prices: await latestPrices(id) });
});
...
router.get('/:id', async (req, res) => {
  ...
  res.json({
    ...
    prices: await latestPrices(req.params.id),
    history,
    receipts: await receiptRows(req.params.id),
  });
});
...
router.get('/:id/receipts', async (req, res) => {
  ...
  if (!product) return res.status(404).json({ error: 'Product not found' });
  res.json(await receiptRows(req.params.id));
});
...
router.post('/:id/receipts', async (req, res) => {
  ...
  try {
    ...
    if (price_snapshot_id) {
      const snapshot = await db.prepare('SELECT id FROM price_snapshots WHERE id = ? AND product_id = ?')
    ...
    const id = nanoid();
    const storedImage = await storeReceiptImage({
    ...
    await db.prepare(`INSERT INTO receipt_images
      (id, product_id, price_snapshot_id, store, image_data, storage_path, mime_type, file_name, file_size, note, uploaded_by)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(
      id,
      req.params.id,
    ...
    const receipt = (await receiptRows(req.params.id)).find(row => row.id === id);
...
router.get('/:id/receipts/:receiptId/image', async (req, res) => {
  try {
    const receipt = await db.prepare(
    `SELECT image_data, storage_path, mime_type FROM receipt_images
     WHERE id = ? AND product_id = ?`
    ).get(req.params.receiptId, req.params.id);
    if (!receipt) return res.status(404).json({ error: 'Receipt not found' });

    const image = await loadReceiptImage(receipt);
    if (!image) return res.status(404).json({ error: 'Receipt image not found' });
...
router.post('/:id/enrich', async (req, res) => {
  ...
  res.json({
    product: { ...serializeProduct(updated), prices: await latestPrices(req.params.id) },
    enrichment,
  });
});
...
router.put('/:id', async (req, res) => {
  const product = await db.prepare('SELECT * FROM products WHERE id = ? AND created_by = ?')
  ...
  res.json({
    ...serializeProduct(await db.prepare('SELECT * FROM products WHERE id = ?').get(req.params.id)),
    prices: await latestPrices(req.params.id),
  });
});
...
router.post('/:id/refresh', async (req, res) => {
    ...
    if (locId) {
      ...
      if (match?.price) {
        ...
        if (match.imageUrl && !product.image_url) {
          await db.prepare('UPDATE products SET image_url = ? WHERE id = ?').run(match.imageUrl, product.id);
        }
  ...
  res.json({ ...results, prices: await latestPrices(req.params.id) });
});
...
router.post('/:id/price', async (req, res) => {
  ...
  const snapshotId = await insertPriceSnapshot({
    productId: req.params.id,
    store,
    price,
  ...
  res.json({ ok: true, price_snapshot_id: snapshotId, prices: await latestPrices(req.params.id) });
});
